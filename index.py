...
class Atomic(commands.Bot):
    ...
    async def on_message(self, message):
        ...
        directory2 = "./logs/{0}/{1}".format(message.server.name, message.channel.name)
        if not os.path.exists(directory2):
            os.makedirs(directory2)
        with open("./logs/{0}/{1}/messages.txt".format(message.server.name, message.channel.name), "a") as myfile:
           myfile.write("{0} | {1} | {2} | {3}\n".format(message.author.name, message.server.name, message.channel.name, message.content))
